...
from faceweave.typing import OutputVideoEncoder, OutputVideoPreset, Fps
from faceweave.filesystem import is_image, is_video
from faceweave.uis.core import get_ui_components, register_ui_component
from faceweave.vision import detect_image_resolution, create_image_resolutions, detect_video_fps, detect_video_resolution, create_video_resolutions, pack_resolution
...
def render() -> None:
	...
	if is_image(faceweave.globals.target_path):
		output_image_resolution = detect_image_resolution(faceweave.globals.target_path)
		output_image_resolutions = create_image_resolutions(output_image_resolution)
	if is_video(faceweave.globals.target_path):
		output_video_resolution = detect_video_resolution(faceweave.globals.target_path)
		output_video_resolutions = create_video_resolutions(output_video_resolution)
	faceweave.globals.output_path = faceweave.globals.output_path or '.'
	OUTPUT_PATH_TEXTBOX = gradio.Textbox(
		label = wording.get('uis.output_path_textbox'),
		value = faceweave.globals.output_path,
		max_lines = 1
	)
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_image_quality_slider'),
		value = faceweave.globals.output_image_quality,
		step = faceweave.choices.output_image_quality_range[1] - faceweave.choices.output_image_quality_range[0],
		minimum = faceweave.choices.output_image_quality_range[0],
		maximum = faceweave.choices.output_image_quality_range[-1],
		visible = is_image(faceweave.globals.target_path)
	)
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		value = faceweave.globals.output_video_preset,
		visible = is_video(faceweave.globals.target_path)
	)
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_quality_slider'),
		value = faceweave.globals.output_video_quality,
		step = faceweave.choices.output_video_quality_range[1] - faceweave.choices.output_video_quality_range[0],
	...
	OUTPUT_VIDEO_RESOLUTION_DROPDOWN = gradio.Dropdown(
		...
		value = faceweave.globals.output_video_resolution,
		visible = is_video(faceweave.globals.target_path)
	)
	OUTPUT_VIDEO_FPS_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_fps_slider'),
		value = faceweave.globals.output_video_fps,
		step = 0.01,
		minimum = 1,
		maximum = 60,
		visible = is_video(faceweave.globals.target_path)
