...
from faceweave.processors.frame.core import load_frame_processor_module, clear_frame_processors_modules
...
def render() -> None:
	...
	FRAME_PROCESSORS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = sort_frame_processors(faceweave.globals.frame_processors),
		value = faceweave.globals.frame_processors
	)
...
def update_frame_processors(frame_processors : List[str]) -> gradio.CheckboxGroup:
	faceweave.globals.frame_processors = frame_processors
	clear_frame_processors_modules()
	for frame_processor in frame_processors:
		frame_processor_module = load_frame_processor_module(frame_processor)
		if not frame_processor_module.pre_check():
			return gradio.CheckboxGroup()
	return gradio.CheckboxGroup(value = faceweave.globals.frame_processors, choices = sort_frame_processors(faceweave.globals.frame_processors))
