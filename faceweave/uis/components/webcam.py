...
from faceweave.typing import VisionFrame, Face, Fps
from faceweave.face_analyser import get_average_face
...
from faceweave.uis.core import get_ui_component, get_ui_components
...
def start(webcam_mode : WebcamMode, webcam_resolution : str, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	faceweave.globals.face_selector_mode = 'one'
	faceweave.globals.face_analyser_order = 'large-small'
...
def multi_process_capture(source_face : Face, webcam_capture : cv2.VideoCapture, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	with tqdm(desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = faceweave.globals.log_level in [ 'warn', 'error' ]) as progress:
