...
def render() -> None:
	...
	if is_video(faceweave.globals.target_path):
		...
		trim_frame_start_slider_args['value'] = faceweave.globals.trim_frame_start or 0
		trim_frame_start_slider_args['maximum'] = video_frame_total
		trim_frame_start_slider_args['visible'] = True
		trim_frame_end_slider_args['value'] = faceweave.globals.trim_frame_end or video_frame_total
...
def remote_update() -> Tuple[gradio.Slider, gradio.Slider]:
	if is_video(faceweave.globals.target_path):
		video_frame_total = count_video_frame_total(faceweave.globals.target_path)
		faceweave.globals.trim_frame_start = None
		faceweave.globals.trim_frame_end = None
...
def update_trim_frame_start(trim_frame_start : int) -> None:
	faceweave.globals.trim_frame_start = trim_frame_start if trim_frame_start > 0 else None


def update_trim_frame_end(trim_frame_end : int) -> None:
	video_frame_total = count_video_frame_total(faceweave.globals.target_path)
	faceweave.globals.trim_frame_end = trim_frame_end if trim_frame_end < video_frame_total else None
