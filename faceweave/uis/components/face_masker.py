...
from faceweave.typing import FaceMaskType, FaceMaskRegion
...
def render() -> None:
	...
	has_box_mask = 'box' in faceweave.globals.face_mask_types
	has_region_mask = 'region' in faceweave.globals.face_mask_types
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		value = faceweave.globals.face_mask_types
	)
	with gradio.Group(visible = has_box_mask) as FACE_MASK_BOX_GROUP:
		...
		with gradio.Row():
			...
			FACE_MASK_PADDING_RIGHT_SLIDER = gradio.Slider(
				...
				minimum = faceweave.choices.face_mask_padding_range[0],
				maximum = faceweave.choices.face_mask_padding_range[-1],
				value = faceweave.globals.face_mask_padding[1]
			)
		with gradio.Row():
			FACE_MASK_PADDING_BOTTOM_SLIDER = gradio.Slider(
				...
				minimum = faceweave.choices.face_mask_padding_range[0],
				maximum = faceweave.choices.face_mask_padding_range[-1],
				value = faceweave.globals.face_mask_padding[2]
			)
			FACE_MASK_PADDING_LEFT_SLIDER = gradio.Slider(
				...
				minimum = faceweave.choices.face_mask_padding_range[0],
				maximum = faceweave.choices.face_mask_padding_range[-1],
				value = faceweave.globals.face_mask_padding[3]
			)
	with gradio.Row():
		FACE_MASK_REGION_CHECKBOX_GROUP = gradio.CheckboxGroup(
			...
			value = faceweave.globals.face_mask_regions,
...
def update_face_mask_type(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.Group, gradio.CheckboxGroup]:
	faceweave.globals.face_mask_types = face_mask_types or faceweave.choices.face_mask_types
	has_box_mask = 'box' in face_mask_types
	has_region_mask = 'region' in face_mask_types
	return gradio.CheckboxGroup(value = faceweave.globals.face_mask_types), gradio.Group(visible = has_box_mask), gradio.CheckboxGroup(visible = has_region_mask)


def update_face_mask_blur(face_mask_blur : float) -> None:
	faceweave.globals.face_mask_blur = face_mask_blur
...
def update_face_mask_padding(face_mask_padding_top : int, face_mask_padding_right : int, face_mask_padding_bottom : int, face_mask_padding_left : int) -> None:
	faceweave.globals.face_mask_padding = (face_mask_padding_top, face_mask_padding_right, face_mask_padding_bottom, face_mask_padding_left)


def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	faceweave.globals.face_mask_regions = face_mask_regions or faceweave.choices.face_mask_regions
	return gradio.CheckboxGroup(value = faceweave.globals.face_mask_regions)
