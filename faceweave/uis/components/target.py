...
def render() -> None:
	...
	is_target_image = is_image(faceweave.globals.target_path)
	is_target_video = is_video(faceweave.globals.target_path)
	TARGET_FILE = gradio.File(
		...
		value = faceweave.globals.target_path if is_target_image or is_target_video else None
	)
...
def update(file : File) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		faceweave.globals.target_path = file.name
	...
	faceweave.globals.target_path = None
