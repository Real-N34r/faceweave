...
from faceweave import face_analyser, face_masker, content_analyser, config, process_manager, metadata, logger, wording, voice_extractor
...
from faceweave.common_helper import create_metavar, get_first
from faceweave.execution import encode_execution_providers, decode_execution_providers
from faceweave.normalizer import normalize_output_path, normalize_padding, normalize_fps
...
from faceweave.filesystem import list_directory, get_temp_frame_paths, create_temp, move_temp, clear_temp, is_image, is_video, filter_audio_paths, resolve_relative_path
from faceweave.ffmpeg import extract_frames, merge_video, copy_image, finalize_image, restore_audio, replace_audio
from faceweave.vision import read_image, read_static_images, detect_image_resolution, restrict_video_fps, create_image_resolutions, get_video_frame, detect_video_resolution, detect_video_fps, restrict_video_resolution, restrict_image_resolution, create_video_resolutions, pack_resolution, unpack_resolution
...
def cli() -> None:
	...
	group_execution.add_argument('--execution-thread-count', help = wording.get('help.execution_thread_count'), type = int, default = config.get_int_value('execution.execution_thread_count', '4'), choices = faceweave.choices.execution_thread_count_range, metavar = create_metavar(faceweave.choices.execution_thread_count_range))
	group_execution.add_argument('--execution-queue-count', help = wording.get('help.execution_queue_count'), type = int, default = config.get_int_value('execution.execution_queue_count', '1'), choices = faceweave.choices.execution_queue_count_range, metavar = create_metavar(faceweave.choices.execution_queue_count_range))
	# memory
	...
	group_memory.add_argument('--system-memory-limit', help = wording.get('help.system_memory_limit'), type = int, default = config.get_int_value('memory.system_memory_limit', '0'), choices = faceweave.choices.system_memory_limit_range, metavar = create_metavar(faceweave.choices.system_memory_limit_range))
	# face analyser
	group_face_analyser = program.add_argument_group('face analyser')
	group_face_analyser.add_argument('--face-analyser-order', help = wording.get('help.face_analyser_order'), default = config.get_str_value('face_analyser.face_analyser_order', 'left-right'), choices = faceweave.choices.face_analyser_orders)
	group_face_analyser.add_argument('--face-analyser-age', help = wording.get('help.face_analyser_age'), default = config.get_str_value('face_analyser.face_analyser_age'), choices = faceweave.choices.face_analyser_ages)
	group_face_analyser.add_argument('--face-analyser-gender', help = wording.get('help.face_analyser_gender'), default = config.get_str_value('face_analyser.face_analyser_gender'), choices = faceweave.choices.face_analyser_genders)
	group_face_analyser.add_argument('--face-detector-model', help = wording.get('help.face_detector_model'), default = config.get_str_value('face_analyser.face_detector_model', 'yoloface'), choices = faceweave.choices.face_detector_set.keys())
	...
	group_face_analyser.add_argument('--face-detector-score', help = wording.get('help.face_detector_score'), type = float, default = config.get_float_value('face_analyser.face_detector_score', '0.5'), choices = faceweave.choices.face_detector_score_range, metavar = create_metavar(faceweave.choices.face_detector_score_range))
	group_face_analyser.add_argument('--face-landmarker-score', help = wording.get('help.face_landmarker_score'), type = float, default = config.get_float_value('face_analyser.face_landmarker_score', '0.5'), choices = faceweave.choices.face_landmarker_score_range, metavar = create_metavar(faceweave.choices.face_landmarker_score_range))
	# face selector
	...
	group_face_selector.add_argument('--reference-face-distance', help = wording.get('help.reference_face_distance'), type = float, default = config.get_float_value('face_selector.reference_face_distance', '0.6'), choices = faceweave.choices.reference_face_distance_range, metavar = create_metavar(faceweave.choices.reference_face_distance_range))
	...
	group_face_mask.add_argument('--face-mask-types', help = wording.get('help.face_mask_types').format(choices = ', '.join(faceweave.choices.face_mask_types)), default = config.get_str_list('face_mask.face_mask_types', 'box'), choices = faceweave.choices.face_mask_types, nargs = '+', metavar = 'FACE_MASK_TYPES')
	group_face_mask.add_argument('--face-mask-blur', help = wording.get('help.face_mask_blur'), type = float, default = config.get_float_value('face_mask.face_mask_blur', '0.3'), choices = faceweave.choices.face_mask_blur_range, metavar = create_metavar(faceweave.choices.face_mask_blur_range))
	...
	group_face_mask.add_argument('--face-mask-regions', help = wording.get('help.face_mask_regions').format(choices = ', '.join(faceweave.choices.face_mask_regions)), default = config.get_str_list('face_mask.face_mask_regions', ' '.join(faceweave.choices.face_mask_regions)), choices = faceweave.choices.face_mask_regions, nargs = '+', metavar = 'FACE_MASK_REGIONS')
	# frame extraction
	...
	group_output_creation.add_argument('--output-image-quality', help = wording.get('help.output_image_quality'), type = int, default = config.get_int_value('output_creation.output_image_quality', '80'), choices = faceweave.choices.output_image_quality_range, metavar = create_metavar(faceweave.choices.output_image_quality_range))
	...
	group_output_creation.add_argument('--output-video-quality', help = wording.get('help.output_video_quality'), type = int, default = config.get_int_value('output_creation.output_video_quality', '80'), choices = faceweave.choices.output_video_quality_range, metavar = create_metavar(faceweave.choices.output_video_quality_range))
	...
	# frame processors
	available_frame_processors = list_directory('faceweave/processors/frame/modules')
...
def apply_args(program : ArgumentParser) -> None:
	...
	faceweave.globals.output_path = args.output_path
	# misc
	faceweave.globals.force_download = args.force_download
	...
	# memory
	faceweave.globals.video_memory_strategy = args.video_memory_strategy
	faceweave.globals.system_memory_limit = args.system_memory_limit
	# face analyser
	...
	if args.face_detector_size in faceweave.choices.face_detector_set[args.face_detector_model]:
	...
	faceweave.globals.face_landmarker_score = args.face_landmarker_score
	# face selector
	...
	if is_image(args.target_path):
		...
		if args.output_image_resolution in output_image_resolutions:
			faceweave.globals.output_image_resolution = args.output_image_resolution
		else:
			faceweave.globals.output_image_resolution = pack_resolution(output_image_resolution)
	faceweave.globals.output_video_encoder = args.output_video_encoder
	faceweave.globals.nsfw = args.nsfw
	faceweave.globals.output_video_preset = args.output_video_preset
	...
	if is_video(args.target_path):
		...
		if args.output_video_resolution in output_video_resolutions:
			faceweave.globals.output_video_resolution = args.output_video_resolution
		else:
			faceweave.globals.output_video_resolution = pack_resolution(output_video_resolution)
	if args.output_video_fps or is_video(args.target_path):
		faceweave.globals.output_video_fps = normalize_fps(args.output_video_fps) or detect_video_fps(args.target_path)
...
def run(program : ArgumentParser) -> None:
	...
	if faceweave.globals.system_memory_limit > 0:
		limit_system_memory(faceweave.globals.system_memory_limit)
	if faceweave.globals.force_download:
		force_download()
		return
...
def process_image(start_time : float) -> None:
	normed_output_path = normalize_output_path(faceweave.globals.target_path, faceweave.globals.output_path)
	...
	temp_image_resolution = pack_resolution(restrict_image_resolution(faceweave.globals.target_path, unpack_resolution(faceweave.globals.output_image_resolution)))
	logger.info(wording.get('copying_image').format(resolution = temp_image_resolution), __name__.upper())
	if copy_image(faceweave.globals.target_path, normed_output_path, temp_image_resolution):
	...
	for frame_processor_module in get_frame_processors_modules(faceweave.globals.frame_processors):
		...
		frame_processor_module.process_image(faceweave.globals.source_paths, normed_output_path, normed_output_path)
		frame_processor_module.post_process()
	if is_process_stopping():
		return
	# finalize image
	logger.info(wording.get('finalizing_image').format(resolution = faceweave.globals.output_image_resolution), __name__.upper())
	if finalize_image(normed_output_path, faceweave.globals.output_image_resolution):
...
def process_video(start_time : float) -> None:
	...
	# extract frames
	process_manager.start()
	temp_video_resolution = pack_resolution(restrict_video_resolution(faceweave.globals.target_path, unpack_resolution(faceweave.globals.output_video_resolution)))
	temp_video_fps = restrict_video_fps(faceweave.globals.target_path, faceweave.globals.output_video_fps)
	logger.info(wording.get('extracting_frames').format(resolution = temp_video_resolution, fps = temp_video_fps), __name__.upper())
	if extract_frames(faceweave.globals.target_path, temp_video_resolution, temp_video_fps):
	...
	if temp_frame_paths:
		...
		return
	# merge video
	logger.info(wording.get('merging_video').format(resolution = faceweave.globals.output_video_resolution, fps = faceweave.globals.output_video_fps), __name__.upper())
	if merge_video(faceweave.globals.target_path, faceweave.globals.output_video_resolution, faceweave.globals.output_video_fps):
	...
	if faceweave.globals.skip_audio:
		...
		move_temp(faceweave.globals.target_path, normed_output_path)
	else:
		if 'lip_syncer' in faceweave.globals.frame_processors:
			source_audio_path = get_first(filter_audio_paths(faceweave.globals.source_paths))
			if source_audio_path and replace_audio(faceweave.globals.target_path, source_audio_path, normed_output_path):
			...
			if restore_audio(faceweave.globals.target_path, normed_output_path, faceweave.globals.output_video_fps):
				logger.debug(wording.get('restoring_audio_succeed'), __name__.upper())
			else:
				if is_process_stopping():
					return
