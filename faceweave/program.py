...
from faceweave import config, logger, metadata, state_manager, wording
...
def create_face_analyser_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	...
	group_face_analyser.add_argument('--face-detector-angles', help = wording.get('help.face_detector_angles'), type = int, default = config.get_int_list('face_analyser.face_detector_angles', '0'), choices = faceweave.choices.face_detector_angles, nargs = '+', metavar = 'FACE_DETECTOR_ANGLES')
...
def create_face_selector_program() -> ArgumentParser:
	...
	group_face_selector.add_argument('--face-selector-order', help = wording.get('help.face_selector_order'), default = config.get_str_value('face_selector.face_selector_order', 'left-right'), choices = faceweave.choices.face_selector_orders)
	group_face_selector.add_argument('--face-selector-age', help = wording.get('help.face_selector_age'), default = config.get_str_value('face_selector.face_selector_age'), choices = faceweave.choices.face_selector_ages)
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-mask-blur', help = wording.get('help.face_mask_blur'), type = float, default = config.get_float_value('face_masker.face_mask_blur', '0.3'), choices = faceweave.choices.face_mask_blur_range, metavar = create_metavar(faceweave.choices.face_mask_blur_range))
