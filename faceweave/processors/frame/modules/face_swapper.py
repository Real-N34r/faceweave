...
import faceweave.processors.frame.core as frame_processors
from faceweave import config, process_manager, logger, wording
from faceweave.execution import apply_execution_provider_options
from faceweave.face_analyser import get_one_face, get_average_face, get_many_faces, find_similar_faces, clear_face_analyser
from faceweave.face_masker import create_static_box_mask, create_occlusion_mask, create_region_mask, clear_face_occluder, clear_face_parser
from faceweave.face_helper import warp_face_by_face_landmark_5, paste_back
from faceweave.face_store import get_reference_faces
from faceweave.content_analyser import clear_content_analyser
from faceweave.normalizer import normalize_output_path
from faceweave.thread_helper import thread_lock, conditional_thread_semaphore
from faceweave.typing import Face, Embedding, VisionFrame, UpdateProgress, ProcessMode, ModelSet, OptionsWithModel, QueuePayload
from faceweave.filesystem import is_file, is_image, has_image, is_video, filter_image_paths, resolve_relative_path
from faceweave.download import conditional_download, is_download_done
from faceweave.vision import read_image, read_static_image, read_static_images, write_image
from faceweave.processors.frame.typings import FaceSwapperInputs
from faceweave.processors.frame import globals as frame_processors_globals
from faceweave.processors.frame import choices as frame_processors_choices
...
def get_frame_processor() -> Any:
	...
	with thread_lock():
		...
		if FRAME_PROCESSOR is None:
			...
			FRAME_PROCESSOR = onnxruntime.InferenceSession(model_path, providers = apply_execution_provider_options(faceweave.globals.execution_providers))
...
def apply_args(program : ArgumentParser) -> None:
	...
	if args.face_swapper_model == 'blendswap_256':
		faceweave.globals.face_recognizer_model = 'arcface_blendswap'
	...
	if args.face_swapper_model == 'simswap_256' or args.face_swapper_model == 'simswap_512_unofficial':
		faceweave.globals.face_recognizer_model = 'arcface_simswap'
	if args.face_swapper_model == 'uniface_256':
		faceweave.globals.face_recognizer_model = 'arcface_uniface'
...
def post_check() -> bool:
	...
	if not faceweave.globals.skip_download and not is_download_done(model_url, model_path):
...
def pre_process(mode : ProcessMode) -> bool:
	if not has_image(faceweave.globals.source_paths):
		logger.error(wording.get('select_image_source') + wording.get('exclamation_mark'), NAME)
		return False
	source_image_paths = filter_image_paths(faceweave.globals.source_paths)
	...
	for source_frame in source_frames:
		if not get_one_face(source_frame):
			...
			return False
	if mode in [ 'output', 'preview' ] and not is_image(faceweave.globals.target_path) and not is_video(faceweave.globals.target_path):
		logger.error(wording.get('select_image_or_video_target') + wording.get('exclamation_mark'), NAME)
		return False
	if mode == 'output' and not normalize_output_path(faceweave.globals.target_path, faceweave.globals.output_path):
...
def post_process() -> None:
	...
	if faceweave.globals.video_memory_strategy == 'strict' or faceweave.globals.video_memory_strategy == 'moderate':
		clear_model_initializer()
		clear_frame_processor()
	if faceweave.globals.video_memory_strategy == 'strict':
		clear_face_analyser()
...
def swap_face(source_face : Face, target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	...
	if 'box' in faceweave.globals.face_mask_types:
		box_mask = create_static_box_mask(crop_vision_frame.shape[:2][::-1], faceweave.globals.face_mask_blur, faceweave.globals.face_mask_padding)
		crop_mask_list.append(box_mask)
	if 'occlusion' in faceweave.globals.face_mask_types:
		occlusion_mask = create_occlusion_mask(crop_vision_frame)
		crop_mask_list.append(occlusion_mask)
	crop_vision_frame = prepare_crop_frame(crop_vision_frame)
	crop_vision_frame = apply_swap(source_face, crop_vision_frame)
	crop_vision_frame = normalize_crop_frame(crop_vision_frame)
	if 'region' in faceweave.globals.face_mask_types:
		region_mask = create_region_mask(crop_vision_frame, faceweave.globals.face_mask_regions)
...
def prepare_source_frame(source_face : Face) -> VisionFrame:
	model_type = get_options('model').get('type')
	source_vision_frame = read_static_image(faceweave.globals.source_paths[0])
...
def process_frame(inputs : FaceSwapperInputs) -> VisionFrame:
	...
	if faceweave.globals.face_selector_mode == 'many':
	...
	if faceweave.globals.face_selector_mode == 'one':
		target_face = get_one_face(target_vision_frame)
		if target_face:
			target_vision_frame = swap_face(source_face, target_face, target_vision_frame)
	if faceweave.globals.face_selector_mode == 'reference':
		similar_faces = find_similar_faces(reference_faces, target_vision_frame, faceweave.globals.reference_face_distance)
		if similar_faces:
...
def process_frames(source_paths : List[str], queue_payloads : List[QueuePayload], update_progress : UpdateProgress) -> None:
	reference_faces = get_reference_faces() if 'reference' in faceweave.globals.face_selector_mode else None
