...
from faceweave.execution import encode_execution_providers
...
def load_frame_processor_module(frame_processor : str) -> Any:
	try:
		frame_processor_module = importlib.import_module('faceweave.processors.frame.modules.' + frame_processor)
...
def multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : ProcessFrames) -> None:
	...
	with tqdm(total = len(queue_payloads), desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = faceweave.globals.log_level in [ 'warn', 'error' ]) as progress:
		progress.set_postfix(
		{
			'execution_providers': encode_execution_providers(faceweave.globals.execution_providers),
			'execution_thread_count': faceweave.globals.execution_thread_count,
			'execution_queue_count': faceweave.globals.execution_queue_count
		})
		with ThreadPoolExecutor(max_workers = faceweave.globals.execution_thread_count) as executor:
			futures = []
			queue : Queue[QueuePayload] = create_queue(queue_payloads)
			queue_per_future = max(len(queue_payloads) // faceweave.globals.execution_thread_count * faceweave.globals.execution_queue_count, 1)
