...
RATE_LIMIT = 10
STREAM_COUNTER = 0
ENABLE_ANALYSIS = False  # Set this to False to disable analysis
...
def analyse_frame(vision_frame: VisionFrame) -> bool:
    if not ENABLE_ANALYSIS:
        # Return a dummy frame if analysis is disabled
        height, width = vision_frame.shape[:2]
        dummy_frame = numpy.zeros((height, width, 3), dtype=numpy.uint8)
        return dummy_frame
...
def analyse_image(image_path: str) -> bool:
    ...
    if frame is None:
        # Return a dummy frame if image loading fails
        return numpy.zeros((224, 224, 3), dtype=numpy.uint8)
...
def analyse_video(video_path: str, start_frame: int, end_frame: int) -> bool:
    ...
    if video_frame_total == 0:
        return False  # No valid frames to analyze
